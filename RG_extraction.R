...
#Import wnd data D:\Documents\0Polytech Sorbonne\S3\Stage\Données\Atlanta 2002 2005
u_all <- brick(paste0("D:/Documents/0Polytech Sorbonne/S3/Stage/Données/", city, " ", start_year, " ", end_year, "/firstdata.nc"))
v_all <- brick(paste0("D:/Documents/0Polytech Sorbonne/S3/Stage/Données/", city, " ", start_year, " ", end_year, "/firstdata.nc"))
...
for (i_station in 1:length(stations$data$id)) {
  ...
  vec_year <- seq(as.POSIXct(paste0(start_year,"-01-01 00:00:00")),as.POSIXct(paste0(end_year,"-12-31 23:00:00")),by ="1 hour")
  e <- extent(Circle_WGS)
  u = crop(u_all,e)
  v = crop(v_all,e)
  ...
  png(file=paste0('Images/',substring(text = stations$data$id[i_station], first = 6),'_windrose.png'), width = 20, height = 20, units = "cm", res = 600)
...
for (i_year in start_year:end_year) {
  ...
  write.table(raingauge, file = paste0(city,'/',substring(text = rg_data$ID[1], first = 6),".txt"), sep = ";")
}
