...
all_pressure <- c(850) # Air pressure
...
all_start_year <- c(2002, 2006, 2010, 2014)
duration_year <- 3
city = 'Atlanta'
...
#EXO BOUCLER TOUT AVEC TOUTES LES DATES DE DONNES PUIS AVEC TOUTES LES PRESSIONS !!!




stations <- ncdc_stations(datasetid = 'PRECIP_HLY', 
                          startdate = paste0(all_start_year[1], '0101'), 
                          enddate = paste0(all_start_year[1] + (duration_year*4), '1231'), 
                          extent = c(33,-85,34,-83.5), 
                          token = "ynaiHGszPBanriYXUYYVzhEdkuOVyJqe", limit = 1000) # 1000 is upper limit
    ...
    system.time(
      for (i_station in 1:length(stations$data$id)) {
        ...
        # CREATE THE DATAFRAME + NAME COLUMNS + TO BE FILLED WITH THE RESULTS
        ...
        # GET NLCD (or charge yours)
        ras_nlcd <- get_nlcd(template = Circle_WGS,
                             label = stations$data$id[i_station],
                             force.redo = TRUE,
                             year = '2011')
        ...
        ras_nlcd_lcc  <- projectRaster(ras_nlcd, crs = crs(point_planar))
        
        # aggregate for faster run
        ras_nlcd_lcc <- aggregate(ras_nlcd_lcc, fact=100)
        
        png(file=paste0('../analyses/Images/',substring(text = stations$data$id[i_station], first = 6),'_landuse.png'), width = 20, height = 20, units = "cm", res = 600)
        my_plot <- plot(ras_nlcd)
        print(my_plot)
        dev.off()
        
        png(file=paste0('../analyses/Images/',substring(text = stations$data$id[i_station], first = 6),'_landuse_agg.png'), width = 20, height = 20, units = "cm", res = 600)
        ...
        b_upwind = stack(replicate(ras_NA,n=360)) # supperspose all 360 rasters with data urban or rural
        tmin = b_upwind
        values(tmin) = rep(wind_360$upwind_min,each = dim(b_upwind)[1]*dim(b_upwind)[2]) # multiply 1 or 0 by urban surface pourcentage (between 0 & 1)
        tmax = b_upwind
        values(tmax) = rep(wind_360$upwind_max,each = dim(b_upwind)[1]*dim(b_upwind)[2])
        b_upwind[ ((tmin < tmax) & (t > tmin) & (t < tmax)) | ((tmin > tmax) & (((t > tmin) & (t < 360)) | ((t > 0) & (t < tmax))))] = 1 # when Theta englobes the main axe (ex : tmin = 330°; tmax = 15°)
        ...
        for (i in all_pressure) {
          for (j in all_start_year) {
            u_all <- brick(paste0("F:/Downloaded_ERA5/ERA_5_US_U_",i,'_',j,'_',j + duration_year,".nc"), varname="u")
            v_all <- brick(paste0("F:/Downloaded_ERA5/ERA_5_US_V_",i,'_',j,'_',j + duration_year,".nc"), varname="v")
            ...
            vec_year <- seq(as.POSIXct(paste0(j,"-01-01 00:00:00")),as.POSIXct(paste0(j + duration_year,"-12-31 23:00:00")),by ="1 hour")
            e <- extent(Circle_WGS)
            u = crop(u_all,e)
            v = crop(v_all,e)
            vecU = cellStats(u, stat='mean', na.rm=TRUE)
            vecV = cellStats(v, stat='mean', na.rm=TRUE)
            source("../analyses/plot.windrose.R")
            ...
            png(file=paste0('../analyses/Images/',substring(text = stations$data$id[i_station], first = 6),'_windrose.png'), width = 20, height = 20, units = "cm", res = 600)
        ...
        for (i_year in j:j + duration_year) {
        ...
        write.table(raingauge, file = paste0('../analyses/', 
                                             city,'/',substring(text = rg_data$ID[1], 
                                            first = 6),".txt"), sep = ";")
      }
